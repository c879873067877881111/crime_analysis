...
# df.dropna(): 移除 DataFrame df 中包含缺失值（NaN）的行。
# reset_index(drop=True): 重設 DataFrame 的索引，並將舊索引丟棄。
df = df.dropna().reset_index(drop = True)


# print(df.loc[:,['type']].groupby(df['type']).count())
# print(df.loc[:,['oc_county']].groupby(df['oc_county']).count())
# print(df.info())
...
#把每一筆的案件總數值除以28852，再乘以100，計算出該縣市的犯罪占比佔全台灣的幾%
...
for i in range(0, len(crime_type_list.index)):
    ...
    plt.show()
df.to_csv('Descriptive_df110.csv', index=False)
